...
/**
 * Client wrapper for city listing pages.
 * Owns the vehicle-type filter chips (via useSearchParams) so the parent
 * server page never reads searchParams — keeping it statically cached (ISR).
 * Also handles the map / list view toggle.
 */
...
export function CityListingsView({ listings, cityName, citySlug, mapFallbackCenter }: Props) {
  ...
  // Filter client-side; listings with no vehicleTypes set appear in "All" only.
      ...
      {verified.length > 0 && (
        <div>
          ...
          <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {verified.map((l) => <ListingCard key={l.id} listing={l} />)}
          </div>
        </div>
      )}
      {rest.length > 0 && (
        <ListingRowList
          listings={rest}
          title={verified.length > 0 ? `Other rentals in ${cityName}` : `Rentals in ${cityName}`}
          subtitle={
            verified.length > 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="-mx-1 flex flex-wrap items-center gap-1.5 overflow-x-auto px-1 pb-1">
        <Chip href={`/${citySlug}`} active={!activeType}>All types</Chip>
        {VEHICLE_TYPES.map((v) => (
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-surface-soft p-10 text-center">
          ...
          <p className="mt-2 text-sm text-neutral-600">
            ...
            <Link href="/join" className="text-brand-700 hover:underline">list your rental</Link>{" "}
        ...
        <div className="space-y-10">
          ...
          {view === "map" && (
            <div className="space-y-3">
              <ListingsMap listings={filtered} fallbackCenter={mapFallbackCenter} />
              <div className="flex items-center justify-between gap-3 text-xs text-neutral-500">
                <p>
                  {onMap} {onMap === 1 ? "rental" : "rentals"} on the map
                  {missing > 0 && <> · {missing} not shown yet (location pending)</>}
                </p>
                <p className="hidden sm:block">
                  All {filtered.length} {filtered.length === 1 ? "rental is" : "rentals are"} listed below.
                </p>
              </div>
            </div>
          )}
...
function ViewToggle({ view, onChange, onMap }: { view: "list" | "map"; onChange: (v: "list" | "map") => void; onMap: number }) {
  return (
    <div className="flex items-center justify-between gap-3">
      <div role="tablist" aria-label="Listing view" className="inline-flex rounded-full border border-border bg-background p-0.5 text-sm font-medium">
        <button
          type="button" role="tab" aria-selected={view === "list"} onClick={() => onChange("list")}
          className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 transition-colors ${view === "list" ? "bg-brand-900 text-white" : "text-brand-900 hover:bg-brand-50"}`}
        >
          <ListIcon />List
        </button>
        <button
          type="button" role="tab" aria-selected={view === "map"} onClick={() => onChange("map")}
          className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 transition-colors ${view === "map" ? "bg-brand-900 text-white" : "text-brand-900 hover:bg-brand-50"}`}
        >
          <MapIcon />Map
          <span className={`ml-1 rounded-full px-1.5 text-[10px] font-semibold ${view === "map" ? "bg-white/15 text-white" : "bg-brand-100 text-brand-900"}`}>
            {onMap}
          </span>
        </button>
      </div>
