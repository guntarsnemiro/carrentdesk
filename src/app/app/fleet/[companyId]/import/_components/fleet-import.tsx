...
function parseRow(raw: Record<string, unknown>, rowNum: number): ParsedRow {
  ...
  return {
    ...
    odometer_km,
    gov_inspection_date:  parseDateStr(raw["Gov Inspection Last (date)"] ?? raw["Gov Inspection Last (DD/MM/YYYY)"] ?? raw["gov_inspection_date"]),
    gov_inspection_next:  parseDateStr(raw["Gov Inspection Next (date)"] ?? raw["Gov Inspection Next (DD/MM/YYYY)"] ?? raw["gov_inspection_next"]),
    service_date:         parseDateStr(raw["Service Last (date)"]        ?? raw["Service Last (DD/MM/YYYY)"]        ?? raw["service_date"]),
    service_next:         parseDateStr(raw["Service Next (date)"]        ?? raw["Service Next (DD/MM/YYYY)"]        ?? raw["service_next"]),
    insurance_number:     str(raw["Insurance Number"] ?? raw["insurance_number"]) || null,
    insurance_valid_until:parseDateStr(raw["Insurance Valid Until (date)"] ?? raw["Insurance Valid Until (DD/MM/YYYY)"] ?? raw["insurance_valid_until"]),
